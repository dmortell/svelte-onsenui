<script>
	...
	// let items = [];
	// $: items = new Array(length).fill(0).map((e,i) => {return {text:'Item '+(i+1)}})
	$: items = Array.from(Array(length).keys())
	$: console.log(items)

	// $: if (length){
	// 	items = [];
	// 	for (var i=0;i<length; i++) items.push(i);
	// }
...
<Page>
	...
	<LazyList items={items} let:item>
		<ListItem left={item}>Item: {item}</ListItem>
	</LazyList>
</Page>
